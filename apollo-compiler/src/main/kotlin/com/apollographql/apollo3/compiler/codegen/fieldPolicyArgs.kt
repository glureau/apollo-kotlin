...
internal fun GQLTypeDefinition.paginationArgs(
    ...
    relayPaginationArgs(fieldName, schema)
...
private fun GQLTypeDefinition.relayPaginationArgs(fieldName: String, schema: Schema): Set<String> {
  val relayPaginationFields = directives.filter { schema.originalDirectiveName(it.name) == Schema.PAGINATION }.toPaginationFields()
  return if (fieldName !in relayPaginationFields) {
    emptySet()
  } else {
