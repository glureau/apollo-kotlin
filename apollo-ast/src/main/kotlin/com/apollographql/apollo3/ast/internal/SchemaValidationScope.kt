...
import com.apollographql.apollo3.ast.Schema.Companion.PAGINATION_FIELDS
...
internal fun validateSchema(definitions: List<GQLDefinition>, requiresApolloDefinitions: Boolean = false): GQLResult<Schema> {
  ...
  val paginationConnectionTypes = mergedScope.computePaginationConnectionTypes()
  ...
  return if (issues.containsError()) {
    ...
    GQLResult(
        Schema(
            ...
            paginationConnectionTypes = paginationConnectionTypes,
        ),
        issues
    )
  }
...
@ApolloInternal
fun List<GQLDirective>.toPaginationFields(): Set<String> = extractFields(PAGINATION_FIELDS)
...
internal fun ValidationScope.computePaginationConnectionTypes(): Set<String> {
  val connectionTypes = mutableSetOf<String>()
  for (typeDefinition in typeDefinitions.values) {
    val paginationFields = typeDefinition.directives.filter { originalDirectiveName(it.name) == Schema.PAGINATION }.toPaginationFields()
    for (fieldName in paginationFields) {
      val field = typeDefinition.fields.firstOrNull { it.name == fieldName } ?: continue
      connectionTypes += field.type.name
    }
  }
